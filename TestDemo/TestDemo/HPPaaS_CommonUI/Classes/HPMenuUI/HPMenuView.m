...
typedef NS_ENUM(NSInteger, HPMenuViewArrowDirection) {
    HPMenuViewArrowDirectionNone,
    HPMenuViewArrowDirectionUp,
    HPMenuViewArrowDirectionDown,
};

@interface HPMenuView()
@property (strong, readwrite, nonatomic) HPMenuOperation *menu;

@property (nonatomic, assign) HPMenuViewArrowDirection arrowDirection;//箭头方向
@property (nonatomic, strong) NSArray *menuItems;
@property (nonatomic, assign) CGFloat arrowPosition;
@property (nonatomic, strong) UIView *contentView;
@end

@implementation HPMenuView
...
- (void) setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect
{
    ...
    const CGFloat kMenuMargin = 5.f;//弹出框与屏幕最小间隙
    
    if (heightPlusArrow < (outerHeight - rectY1)) { //箭头point下方有足够距离
        
        _arrowDirection = HPMenuViewArrowDirectionUp;
        ...
        //左边距离小于5
        if (point.x < kMenuMargin)
            point.x = kMenuMargin;
        
        //右边距离小于5
        if ((point.x + contentSize.width + kMenuMargin) > outerWidth)
            point.x = outerWidth - contentSize.width - kMenuMargin;
        ...
        _arrowDirection = HPMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = HPMenuViewArrowDirectionNone;
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    HPMenuContainerView *overlay = [HPMenuContainerView menuWithFrame:view.bounds coverStyle:MenuCoverColor_Gray];
//    [[HPMenuContainerView alloc] initWithFrame:view.bounds];
...
- (void)menuItemClick:(UIButton *)sender
{
    ...
    HPMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)makeContentView{
    ...
    //获得最大item的宽度和高度
    for (HPMenuItem *menuItem in _menuItems) {
    ...
    for (HPMenuItem *menuItem in _menuItems) {
        ...
        //自定义样式
        if (_menu.menuBackgroundStyle == HPMenuBackgroundStyleDark) {
            button.backgroundColor = BlackForMenu;
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        }else if(_menu.menuBackgroundStyle == HPMenuBackgroundStyleLight){
        ...
        if (itemNum < _menuItems.count - 1) {
            ...
            if (_menu.menuSegmenteLineStyle == HPMenuSegmenteLineStylefollowContent) {
            ...
            }else if (_menu.menuSegmenteLineStyle == HPMenuSegmenteLineStyleFill) {
...
- (CGPoint) arrowPoint
{
    ...
    if (_arrowDirection == HPMenuViewArrowDirectionUp) {
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition,CGRectGetMinY(self.frame) };
    } else if (_arrowDirection == HPMenuViewArrowDirectionDown) {
